...
struct Replica {
  ...
  Replica(int id, double temp, const std::vector<int> &init_order,
          unsigned int seed)
      : id(id), temp(temp), order(init_order), rng(seed) {}
};

void local_metropolis_sweep(Replica &rep, const GraphAdjacency &adj,
                            int steps) {
  if (rep.order.size() < 2)
    return;
  ...
  for (int step = 0; step < steps; ++step) {
    ...
    int neighbor_cost =
        evaluate_elimination_cost(adj.in_edges, adj.out_edges, neighbor_order);
    int delta_e = neighbor_cost - rep.cost;

    if (delta_e <= 0 ||
        rand_unif(rep.rng) <
            std::exp(-static_cast<double>(delta_e) / rep.temp)) {
...
ParallelTemperingResult run_parallel_tempering_sa(const GraphAdjacency &adj,
                                                  int num_replicas,
  ...
  for (int i = 0; i < num_replicas; ++i) {
    double T = T_min * std::pow(T_max / T_min,
                                static_cast<double>(i) / (num_replicas - 1));
    std::vector<int> init_order = adj.intermediates;
    std::shuffle(init_order.begin(), init_order.end(), master_rng);

    int init_cost =
        evaluate_elimination_cost(adj.in_edges, adj.out_edges, init_order);
  ...
  for (int round = 0; round < exchange_rounds; ++round) {
    ...
    for (int i = 0; i < num_replicas; ++i) {
      threads.emplace_back(local_metropolis_sweep, std::ref(replicas[i]),
                           std::cref(adj), steps_per_sweep);
    }
    ...
    for (int i = start_idx; i < num_replicas - 1; i += 2) {
      ...
      double delta_pt =
          (beta_i - beta_next) * (replicas[i + 1].cost - replicas[i].cost);
...
ParallelTemperingResult run_parallel_tempering_sa_seeded(
  ...
  // Apply minor random adjacent swaps to higher-temperature replicas
  for (int i = 0; i < num_replicas; ++i) {
    if (i > 0) {
      std::uniform_int_distribution<size_t> dist(0, replicas[i].size() - 2);
      for (int k = 0; k < i * 5;
           ++k) { // Perturb slightly based on replica index
        size_t idx = dist(rng);
        std::swap(replicas[i][idx], replicas[i][idx + 1]);
      }
    }
    replica_costs[i] =
        evaluate_elimination_cost(adj.in_edges, adj.out_edges, replicas[i]);
  }

  ParallelTemperingResult result;
  result.best_order = seed_order;
  result.min_cost =
