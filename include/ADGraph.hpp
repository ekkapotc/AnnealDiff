...
public:
  ...
  std::vector<ADVar> compute_gradient_graph(int output_id,
                                            const std::vector<int> &input_ids);
  std::vector<ADVar>
  compute_gradient_graph_custom_order(int output_id,
                                      const std::vector<int> &input_ids,
                                      const std::vector<int> &order);

  ~ADGraph(); // Custom destructor declaration

  void emit_gradient_code(
        int output_id, 
        const std::vector<int>& input_ids, 
        const std::vector<int>& order, 
        std::ostream& out
